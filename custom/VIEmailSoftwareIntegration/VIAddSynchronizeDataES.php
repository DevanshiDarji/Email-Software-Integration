...
class VIAddSynchronizeDataES{
    public function __construct(){
        //First Button!!hint
        $this->addSynchronizeDataES();
    }

    public function addSynchronizeDataES(){
        global $current_user;
        ...
        if(!empty($selResult)){
            ...
            $batchRecord = $targetListSubpanelModule = $backgroundProcessmsg = '';
            ...
            $emsConditionsData = getEMSConditionsData($selResult);
            $conditionMatchedRecord = matchEMSConditionsData($emsConditionsData, $selResult, $recordId='');
            $matchedRecordCount = count($conditionMatchedRecord);
            
            $batchRecord = $selResult['batch_record'];
            $selScheduleData = "SELECT * FROM vi_ems_schedule_sync WHERE ems_software = '$syncSoftware' AND mapping_id = '$id' AND status = 0";
            $selScheduleDataRow = $GLOBALS['db']->fetchOne($selScheduleData);
            $backgroundMessage = translate('LBL_BACKGROUND_PROCESS_MSG1','Administration').' '.translate($suitecrmModule).' '.translate('LBL_BACKGROUND_PROCESS_MSG2','Administration').' '.$syncSoftware.' '.$esModule.' '.translate('LBL_BACKGROUND_PROCESS_MSG3','Administration');

            if($batchManagementStatus == '1' && $matchedRecordCount >= 100){
                if($matchedRecordCount >= 100){
                    if(empty($selScheduleDataRow)){
                        $offset = 0;
                        $limit = $batchRecord;
                    }else{
                        $backgroundProcessmsg = $backgroundMessage;
                    }
                }else{
                    $flag = 1;
                }
            }else if($batchManagementStatus == '1' && $matchedRecordCount == 0){
                if(!empty($selScheduleDataRow)){
                    $backgroundProcessmsg = $backgroundMessage;
                }//end of if
            }else{
            ...
            if($status == "Active"){
                ...
                $moduleBean = BeanFactory::getBean($suitecrmModule);
                if($backgroundProcessmsg == ''){ 
                    if($flag == 1){
                        $moduleList = $moduleBean->get_list('date_entered',$whereQuery ,0,$matchedRecordCount,$matchedRecordCount,0);
                    }else{
                        ...
                        if(isset($moduleList) && !empty($moduleList)){
                                        ...
                                        if($relatedContactESId != ""){
                                            ...
                                            if(isset($resultFetchRelatedContacts['id'])){
                                                ...
                                                if(isset($resultFetchRelatedContacts['results'][0]->id)){
                                                    ...
                                                    $resultFetchRelatedContacts = syncESData("/contacts?api_key=","POST",$syncSoftware,$finalSuiteContactsLeadArray);
                                            ...
                                            $fetchRelatedContacts = syncESData("/contacts?api_key=","POST",$syncSoftware,$finalSuiteContactsLeadArray);
                            ...
                            if($flag != 1){
                                if(!empty($allRecordId)){
                                    insertScheduleSyncData($syncSoftware,$id,$limit,$batchRecord,$suitecrmModule);
                                }
                            }
                        }//end of if
                    }//end of if
                                            ...
                                            if(isset($suitecrmContactsFields) && !empty($suitecrmContactsFields)){
                                                ...
                                                if ($prospectListBean->load_relationship($relatedTableName)) {
                                                    ...
                                                    foreach ((array)$relatedBeans as $k => $val) {
                                                        ...
                                                        $fieldListResponse = syncESData('/api/3/fields','GET',$syncSoftware,$data=array());
                                                        $fieldList = (array)json_decode($fieldListResponse);
                                                        ...
                                                        if(!empty($selectResult)){
                                                            ...
                                                            if(isset($jsonDecodeData->message) && $jsonDecodeData->message != ''){
                                                                ...
                                                                $insertRelatedContact = syncESData("/api/3/contacts","POST",$syncSoftware,$finalSuiteContactsLeadArray);
                                                                ...
                                                                syncESData("/api/3/contacts/".$viEsContactId, "PUT", $syncSoftware,$finalSuiteContactsLeadArray);
                                                            ...
                                                            $updateContactLeadId = addContactsLeadsFromList($contactLeadId, $targetListSubpanelModule, $val->id, $listId, $syncSoftware, $updateRecordData=array(), $finalSuiteContactsLeadArray, $insertContactLeadId, $updateContactLeadId);
                                    ...
                                    foreach ($obj as $k => $value) {
                                        ...
                                        foreach ($suitecrmFields as $keyfield => $vfield){
                                            if($vfield == "email1"){
                                                ...
                                                $fieldValue = getFieldValue($vfield, $moduleBean, $value, $suitecrmModule, $syncSoftware);
                                                $finalSuiteArray[$keyfield] = $fieldValue;
                                            }
                                        }
                                                ...
                                                if ($prospectListBean->load_relationship($relatedTableName)) {
                                                    ...
                                                    foreach ((array)$relatedBeans as $k => $val) {
                                                        ...
                                                        if(!empty($selectResult['vi_es_contact_id'])){
                                                            ...
                                                            $updateContactLeadId = addContactsLeadsFromList($contactLeadId, $targetListSubpanelModule, $val->id, $listId, $syncSoftware, $emailData, $finalContactsArrayInsert, $insertContactLeadId, $updateContactLeadId);
                                                            ...
                                                            if(empty($responseNewContact->code)){
                                                                ...
                                                                if($listId != ""){
                                                                    syncESData("contacts/lists/".$listId."/contacts/add","POST",$syncSoftware,$emailData);    
                                                ...
                                                if ($prospectListBean->load_relationship($relatedTableName)) {
                                                    ...
                                                    foreach ((array)$relatedBeans as $k => $val) {
                                                        ...
                                                        if(!empty($selectResult['vi_es_contact_id'])){
                                                            ...
                                                            $resultRelatedContacts = syncESData("/api/contacts/new","POST",$syncSoftware,$finalSuiteContactsLeadArray);
                ...
                echo json_encode(array('content'=>$html, 'backgroundProcessmsg' => $backgroundProcessmsg));
            }//end of if status
        }//end of if
    }//End of function
}//end of class
new VIAddSynchronizeDataES();
?>