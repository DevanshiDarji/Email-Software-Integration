...
class ViewLicense extends SugarView{
    ...
    public function display()
    {
        ...
        require_once('modules/'.$currentModule.'/license/VIESIntegrationOutfittersLicense.php');
        ...
        $key = VIESIntegrationOutfittersLicense::getKey($currentModule);
        if(!empty($key)) {
        ...
        if(isset($outfitters_config['manage_licensed_users']) && $outfitters_config['manage_licensed_users'] == true) {
            ...
            if(!empty($trimmed_last))
            {
                ...
                $validated = VIESIntegrationOutfittersLicense::doValidate($currentModule);
                $store = array(
                    'last_ran' => time(),
        ...
        $this->ss->display('modules/VIEmailSoftwareIntegrationLicenseAddon/tpls/license.tpl');
    }//end of display
